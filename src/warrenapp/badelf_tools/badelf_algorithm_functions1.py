...
def get_26_neighbors(structure):
    # Get all possible neighbor atoms for each atom within 7 angstroms
    all_neighbors = structure.get_all_neighbors(7)
    neighbors26 = []
...
def get_vertex_site_temp(
    ...
    for t, u, v in permutations:
        ...
        if site is not None:
            sites.append(site)
            transforms.append([t,u,v])
    # We only want to store the planes that are near the original coordinates
    # of the voxel vertices. Here we replace any that result from permutations
    # with None type objects
    if len(sites) != 0:
        if len(sites) == 1:
            return sites[0], transforms[0]
        else:
        # print(sites, transforms)
            return sites, transforms
    else:
        return None
...
def get_vertices_coords_temp(vox_coord, results, lattice, permutations):
    """
    Takes in the coordinates of a voxel and returns the site that matches
    each of its vertices.
    """
    ...
    for key, coord in vertices_coords.items():
        x, y, z = coord
        try:
            sites, transforms = get_vertex_site_temp(x, y, z, results, permutations, lattice)
            vertex_row = [key, transforms, sites]
            vertices_sites.loc[len(vertices_sites.index)] = vertex_row
        # If we can't find a site, return an empty row for this key
        except:
            vertex_row = [key, None, None]
...
def get_intersections_df(
    sites, results, edges, transformed_coords_real, intersections_df, allow_point_intercept=False
):
    """
    ...
    for site in sites["site"]:
        ...
        for neighbor, values in results[site].items():
            ...
            if neighbor_index in sites_to_search and neighbor_index != site:
            # if neighbor_index is not None:
                plane_point = values["real_min_point"]
                plane_vector = values["normal_vector"]
                # print(plane_point, plane_vector)
                ...
                for edge in edges:
                    ...
                    intersection = get_vector_plane_intersection(
                        point0, point1, plane_point, plane_vector, allow_point_intercept=allow_point_intercept
                    )
...
def get_site_volume_ratio(x, y, z, results, lattice, permutations, voxel_volume):
    ...
    vertices_coords, vertices_sites = get_vertices_coords_temp(
    ...
    # most_common_site = vertices_sites["site"].value_counts().idxmax()
    
    # Sometimes the site search finds vertices that are assigned to two different
    # sites depending on the transformation. I can't think of a reason this would
    # happen unless they are directly on a dividing plane. If that's true, then
    # they should be assigned to whichever site is dominant so I change from
    # a list of sites to the most dominant site
    
    # Get the sites for vertices that only found one site
    found_sites1 = vertices_sites["site"][vertices_sites["site"].apply(isinstance, args=(int,))].to_list()
    
    # If there are none that only found one site, add any sites that are in lists
    if len(found_sites1) == 0:
        found_sites2 = []
        for row in vertices_sites.iterrows():
            sites = row[1]["site"]
            if isinstance(sites, list):
                found_sites2.extend(sites)
            else:
                pass
    else:
        found_sites2 = found_sites1.copy()
    # If any sites were found, change all lists to the most common site. If none
    # were found, all vertices found no sites and the voxel is passed on to the
    # multiplane method.
    if len(found_sites2) > 0 and len(found_sites1) != 8:
        most_common_site = mode(found_sites2)
        for row in vertices_sites.iterrows():
            if isinstance(row[1]["site"], list):
                try:
                    index = row[1]["site"].index(most_common_site)
                except:
                    index = 0
                vertices_sites.loc[row[0], "site"] = most_common_site
                vertices_sites.loc[row[0], "transform"] = row[1]["transform"][index]
    ...
    if len(intersections_df) == 0:
        ...
        for transform in transforms:
            ...
            intersections_df = get_intersections_df(
                sites, results, edges, transformed_coords_real, intersections_df, allow_point_intercept=True
            )
    ...
    if len(sites) == 1:
        ...
        if voxel_site is not None:
            if len(intersections_df) == 0 or len(intersections_df) == 1:
