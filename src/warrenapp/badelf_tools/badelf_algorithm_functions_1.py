...
def get_sign(point, unit_vector, site_pos, lattice):
...
def get_voxel_center_and_vertices(lattice):
    """
    Finds the center and vertices of a voxel with its corner at the origin.
    ...
    # In general, voxels positions are given by their center in this algorithm.
    # We should shift each point here so that the origin is the center of the
    # voxel.
    unshifted = [center, end, vox_a, vox_b, vox_c, vox_a1, vox_b1, vox_c1, end]
    shifted = []
    for vector in unshifted:
        new_vector = [x - x1 for x, x1 in zip(vector, center)]
        shifted.append(new_vector)
    return shifted


def get_max_voxel_dist(lattice):
    """
    Function for determining the maximum distance a voxel can be from a
    dividing plane that still allows for intersection with that plane.
    """
    ...
    # We can't say for sure which one is the largest so we find all of their
    # distances and return the max
    voxel_points = get_voxel_center_and_vertices(lattice)
    max_distance = max([np.linalg.norm(vector) for vector in voxel_points])

    return max_distance


def get_voxel_vertices_real(real_coord, lattice):
    """
    Takes in the coordinate of a voxel and gives the 8 vertices that belong to
    it. Note that the real coordinates of each voxel is at the center of the
    voxel.
    """
    # first we get the cartesian coordinates for the vectors that define the
    # vertices of a voxel if the center were at the origin.
    voxel_vectors = get_voxel_center_and_vertices(lattice)
    # now we want to make a list of the 8 vertices that make up the voxel
    voxel_vertices = []
    for vertex in voxel_vectors[1:]:
        new_vertex = [x + x1 for x, x1 in zip(real_coord, vertex)]
        voxel_vertices.append(new_vertex)
...
def get_matching_site(pos, results, lattice, max_distance):
    ...
    for site, neighs in results.items():
        ...
        for neigh, values in neighs.items():
            ...
            sign, distance = get_sign(point, normal_vector, pos, lattice)
            # if sign ever doesn't match, then the site is wrong and we move
            # on to the next one after setting matched to false
...
def get_site_neighbor_results(neigh: list, lattice: dict, site_pos: dict, grid):
    ...
    normal_vector = get_unit_vector(site_pos, neigh_pos, lattice)
    ...
    sign, distance = get_sign(min_pos_real, normal_vector, site_pos, lattice)
